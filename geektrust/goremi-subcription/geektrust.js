...
const printInfo = () => {
	...
	for (j = 0; j < planList.length; j++) {
		console.log(
			'RENEWAL_REMINDER ' + planList[j].type + ' ' + planList[j].endDate
		);
	}
...
const addTop = (device, num) => {
	...
	let checkSubscribe = TOPUPLIST.find((item) => item == device + '_' + num);
	if (checkSubscribe) {
...
const subScrip = (type, plan) => {
	...
	let endDate = moment(subscriptionPlan.date, 'DD-MM-YYYY')
		.add(month, 'M')
		.format('DD-MM-YYYY');
	let obj = {
		type,
